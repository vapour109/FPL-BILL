...
import { LEAGUE_ROOM_CODE, NAME_STORAGE_KEY } from "@/lib/room";
...
export default function LeaguePage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!r) {
        // The room is created by /api/room on first load; treat its absence as
        // "not ready yet" rather than a dead end, since there is no wrong URL to land on.
        setNotFound(true);
        return;
      }
      setNotFound(false);
  ...
  useEffect(() => {
    let cancelled = false;
    // Ensure the league's room row exists before the first read.
  ...
  function joinAs(name: string) {
    const trimmed = name.trim();
    if (!trimmed) return;
    writeStoredName(NAME_STORAGE_KEY, trimmed);
  }
  ...
  function forgetName() {
    writeStoredName(NAME_STORAGE_KEY, "");
    setNameInput("");
  }
  ...
  if (!room) {
    return (
      <div className="min-h-screen flex items-center justify-center px-6 text-center text-sm" style={{ color: "var(--ink-soft)" }}>
        <div>
          <p>{loadError ? "Couldn't load the league." : "Loading…"}</p>
          {loadError && <p className="mt-2" style={{ color: "var(--money)" }}>{loadError}</p>}
          {notFound && !loadError && <p className="mt-2">Setting up the league…</p>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-5 py-6">
        ...
        {tab === "roster" &&
          (!myName ? (
            <NamePrompt value={nameInput} onChange={setNameInput} onSubmit={() => joinAs(nameInput)} />
          ) : me ? (
            <RosterTab
              room={room}
              manager={me}
            ...
            <p className="text-sm" style={{ color: "var(--ink-soft)" }}>Setting you up as {myName}…</p>
          ))}
...
// Asked for only when someone goes to log a gameweek — charges have to be
// attributed to a manager. Viewing the bill needs no name at all.
function NamePrompt({
  value,
  onChange,
  onSubmit,
  ...
  return (
    <div className="max-w-sm">
      ...
      <p className="text-sm mb-5" style={{ color: "var(--ink-soft)" }}>
        Just so your charges land on the right manager. Anyone can read the bill without this.
      </p>
      <input
        value={value}
